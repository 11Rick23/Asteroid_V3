...
まずコードの意図を読み取り、「機能要件を満たしていること」をテストする。
そのうえで、権限、guild scope、副作用の抑止、冪等性、ログ、エラー処理、安定性などの非機能要件が読み取れる場合は、それもテスト対象にする。
非機能要件として守るべき性質が存在しない場合は、そのことを確認したうえで、無理に非機能要件のテストを作らない。
...
コードを読んでも非機能要件として守るべき性質が見当たらない場合は、機能要件を中心に、壊れると困る振る舞いを代表ケースで確認する。
...
- `# Given` / `# When` / `# Then` は、前提・実行・検証が分かれる場合は原則として別々に書く。
- 例外検証、schema 定義の確認、ごく小さい純粋関数の検証など、実行と検証が自然に一体化する場合は `# When / Then` や `# Given / When` のように併記してよい。
- Given-When-Then の前に、テストしている要件を `# 機能要件：...` または `# 非機能要件：...` のコメントで明示する。
- 1 つのテストで機能要件と非機能要件の両方を守る場合は、両方のコメントを書く。
- 非機能要件が存在しないと判断したテストでは、`# 非機能要件：...` を作らず、`# 機能要件：...` だけを書く。
