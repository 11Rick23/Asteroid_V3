...
class StarGradeModel(Base):
    ...
    prestige: Mapped[int] = mapped_column(TINYINT(unsigned=True), default=0)
    grade: Mapped[int] = mapped_column(TINYINT(unsigned=True), default=0)
    shard: Mapped[int] = mapped_column(INTEGER(unsigned=True), default=0)
    text_shard: Mapped[int] = mapped_column(INTEGER(unsigned=True), default=0)
