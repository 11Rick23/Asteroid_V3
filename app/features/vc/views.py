...
def member_mentions(members: list[discord.Member]) -> str:
    return " ".join(member.mention for member in members) or messages.NONE


class NameChangeModal(GuildScopedModal, title=messages.NAME_MODAL_TITLE):
    vc_name = discord.ui.TextInput(label=messages.NEW_NAME_INPUT_LABEL, max_length=100)
...
class ChangeNameButton(discord.ui.Button["VoiceControlView"]):
    def __init__(self, service: VoiceCreateService, channel_id: int | None = None):
        ...
        super().__init__(
            label=messages.NAME_CHANGE_COOLDOWN_LABEL if is_rate_limited else messages.NAME_CHANGE_LABEL,
...
class TogglePrivacyButton(discord.ui.Button["VoiceControlView"]):
    def __init__(self, service: VoiceCreateService, channel: discord.VoiceChannel | None = None):
        ...
        label = messages.MAKE_PUBLIC_LABEL if is_private else messages.MAKE_PRIVATE_LABEL
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.followup.send(
            (messages.PRIVATE_COMPLETED if private else messages.PUBLIC_COMPLETED),
        )
...
class UserLimitSelect(discord.ui.Select["VoiceControlView"]):
    def __init__(self, service: VoiceCreateService, channel: discord.VoiceChannel | None = None):
        ...
        options = [
            discord.SelectOption(label=messages.UNLIMITED_LABEL, value="0", default=current_limit == 0),
            *[
        ...
        super().__init__(
            placeholder=messages.LIMIT_PLACEHOLDER,
            options=options,
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await self.service.refresh_control_message(interaction, channel)
        await interaction.followup.send(
            messages.modal_limit_changed(actor_display_name=interaction.user.display_name, limit=limit),
        )
...
class BlockedUserSelect(discord.ui.UserSelect["VoiceControlView"]):
    def __init__(self, service: VoiceCreateService, channel: discord.VoiceChannel | None = None):
        ...
        super().__init__(
            ...
            placeholder=messages.BLOCK_PLACEHOLDER,
            min_values=0,
            max_values=25,
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await self.service.refresh_control_message(interaction, channel)
        embed = discord.Embed(
            title=messages.BLOCKED_TITLE,
            description=messages.block_result(
                blocked_mentions=member_mentions(blocked_members),
                unblocked_mentions=member_mentions(unblocked_members),
            ),
        )
...
class OperatorUserSelect(discord.ui.UserSelect["VoiceControlView"]):
    def __init__(self, service: VoiceCreateService, channel: discord.VoiceChannel | None = None):
        ...
        super().__init__(
            ...
            placeholder=messages.OP_PLACEHOLDER,
            min_values=0,
            max_values=25,
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await self.service.refresh_control_message(interaction, channel)
        embed = discord.Embed(
            title=messages.OPPED_TITLE,
            description=messages.op_result(
                opped_mentions=member_mentions(oped_members),
                deopped_mentions=member_mentions(deoped_members),
            ),
        )
        await interaction.followup.send(embed=embed)
...
class VoiceControlView(GuildScopedLayoutView):
    def __init__(
        ...
        *,
        ...
        super().__init__(timeout=None)
        channel_name = channel.name if channel else messages.CONTROL_FALLBACK_TITLE
        ...
        self.add_item(
            discord.ui.Container(
                ...
                discord.ui.TextDisplay(messages.LIMIT_SECTION_TITLE),
                discord.ui.ActionRow(UserLimitSelect(service, channel)),
                discord.ui.Separator(),
                discord.ui.TextDisplay(messages.BLOCK_SECTION_TITLE),
                discord.ui.ActionRow(BlockedUserSelect(service, channel)),
                discord.ui.Separator(),
                discord.ui.TextDisplay(messages.OP_SECTION_TITLE),
