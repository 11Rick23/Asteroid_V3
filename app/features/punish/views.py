...
class PermRoleSelect(discord.ui.Select):
    def __init__(
        ...
        super().__init__(
            placeholder=messages.DISROBE_PLACEHOLDER,
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if (
            ...
            await interaction.response.send_message(messages.PERMISSION_REQUIRED, ephemeral=True)
            return
        ...
        log_punishment_action(messages.PUNISHMENT_DISROBE, interaction, self.target.id, probation=self.probation)
        ...
        await send_punish_message(
            ...
            self.target,
            self.reason,
            messages.disrobe_punishment(role_names=[role.name for role in roles]),
            self.probation,
        )
        await interaction.response.edit_message(content=messages.SENT, view=None)
