...
shard_group = app_commands.Group(name="shard", description=messages.SHARD_GROUP_DESCRIPTION)
reward_group = app_commands.Group(name="reward", description=messages.REWARD_GROUP_DESCRIPTION, parent=shard_group)
RANKING_PAGE_SIZE = 5


@shard_group.command(name="top", description=messages.SHARD_TOP_DESCRIPTION)
async def top(interaction: discord.Interaction) -> None:
    ...
    pages = build_shard_ranking_pages(
        bot,
        star_grades,
        title=messages.SHARD_RANKING_TITLE,
        description=messages.shard_ranking_description(
            heading=messages.CURRENT_SHARD_RANKING_HEADING,
...
@shard_group.command(name="xp_boost", description=messages.XP_BOOST_DESCRIPTION)
async def xp_boost(interaction: discord.Interaction) -> None:
    ...
    if len(xp_boosts) == 0:
        ...
        await interaction.response.send_message(messages.NO_XP_BOOST)
        return
    logger.debug(
        "XPブースト一覧を表示しました: command=/shard xp_boost "
        f"guild_id={interaction.guild_id} channel_id={interaction.channel_id} "
    ...
    for start in range(0, len(xp_boosts), 10):
        ...
        for boost in xp_boosts[start : start + 10]:
            ...
            entries.append(
                messages.xp_boost_entry(
                    name=boost.name,
                    role_mention=role.mention if role else messages.ROLE_NOT_FOUND,
                    boost_amount=boost.boost_amount,
                    end_time=discord.utils.format_dt(boost.boost_end_time, "f")
                    if boost.boost_end_time
                    else messages.UNLIMITED,
                )
            )
        pages.append(build_text_container(messages.xp_boost_list(entries=entries)))
...
@reward_group.command(name="grade", description=messages.GRADE_REWARD_DESCRIPTION)
async def reward_grade(interaction: discord.Interaction) -> None:
    ...
    await interaction.response.send_message(
        view=build_text_view(
            messages.grade_reward_list(entries=entries),
            accent_color=AsteroidColor.INFO,
        )
    )


@reward_group.command(name="prestige", description=messages.PRESTIGE_REWARD_DESCRIPTION)
async def reward_prestige(interaction: discord.Interaction) -> None:
    ...
    for prestige_role in prestige_roles:
        ...
        entries.append(
            messages.prestige_reward_entry(
    ...
    await interaction.response.send_message(
        view=build_text_view(
            messages.prestige_reward_list(entries=entries),
            accent_color=AsteroidColor.INFO,
        )
    )
