...
leveling_admin_group = app_commands.Group(name="leveling_admin", description="レベリング管理コマンド")
xp_boost_group = app_commands.Group(name="xp_boost", description="経験値ブースト設定", parent=leveling_admin_group)
...
@xp_boost_group.command(name="add", description="経験値ブーストを追加します")
async def xp_boost_add(interaction: discord.Interaction, role: discord.Role, name: str, amount: int) -> None:
    ...
    await interaction.response.send_message("経験値ブーストを追加しました。")


@xp_boost_group.command(name="delete", description="経験値ブーストを削除します")
async def xp_boost_delete(interaction: discord.Interaction, role: discord.Role) -> None:
    ...
    await interaction.response.send_message("経験値ブーストを削除しました。")


@admin_shard_group.command(name="add", description="ユーザーにシャードを追加します")
async def add_shard(interaction: discord.Interaction, user: discord.Member, shard_type: str, amount: int) -> None:
    bot = get_bot(interaction)
    ...
    await interaction.response.send_message(
        content=f"{user.mention}に`{humanize_number(amount)}`{shard_type}シャードを付与しました\n{grade_up_amount}回グレードアップしました、{prestige_up_amount}回プレステージしました",
...
async def remove_shard(interaction: discord.Interaction, user: discord.Member, shard_type: str, amount: int) -> None:
    bot = get_bot(interaction)
    star_grade = await bot.db.star_grades.get_star_grade(user.id) or await bot.db.star_grades.create_star_grade(
        user.id
    )
    ...
    await interaction.response.send_message(
        content=f"{user.mention}から`{humanize_number(amount)}`{shard_type}シャードを減らしました",
        embed=build_star_grade_embed(user, star_grade),
    )
    await sync_grade_prestige_role(bot, user, star_grade)
