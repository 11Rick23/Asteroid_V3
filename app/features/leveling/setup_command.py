...
@app_commands.command(name="claim_voice_xp_button", description=messages.SETUP_VOICE_XP_DESCRIPTION)
...
async def claim_voice_xp_button(interaction: discord.Interaction) -> None:
    ...
    if channel is None:
        await interaction.response.send_message(messages.CHANNEL_NOT_MESSAGEABLE, ephemeral=True)
        return
    ...
    await interaction.response.send_message(messages.VOICE_XP_BUTTON_INSTALLED, ephemeral=True)
