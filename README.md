...
管理者用の `/migrate 移行元 移行先` をサーバーのテキストチャンネルで実行すると、
コマンドへの直接レスポンスにComponents V2の移行前チェックを表示し、同じメッセージを確認画面へ更新します。
「確定して移行」で実行し、5分で確認の有効期限が切れます。
画面では絵文字・メンション・強調で変更点を示し、txtには名前・ID・数量をプレーンテキストで記録します。
確定・キャンセル後も確認内容と実行結果を通常メッセージとして残します。操作できるのは実行した管理者だけです。
処理中・処理後も同じメッセージを編集し、Components V2で状態・移行内容・「レベル復元用のコマンド」を残します。
権限不足・入力エラーなどの応答はephemeralで表示します。移行記録は引き続きチャンネルに残します。
...
- 移行先にないロールは追加して移行元から外し、両方が持つ共通ロールは移行元にも残します。
  権限不足・Bot以上のロール・連携管理・削除済みロールは
  プレビューと詳細ファイルに対象外として表示し、除外して移行を続けます。操作できないロールは移行元に残します。
- 誕生日は移行元の値で上書きします。フリカテ権限は殿堂入り・フリー・マイナー・アーカイブの各設定カテゴリにある
  テキストチャンネルのメンバー個別設定が対象です。誕生日・権限の各項目は、移行元が未設定なら移行先の値を保持し、
  両方に設定がある場合だけ移行元を優先します。
